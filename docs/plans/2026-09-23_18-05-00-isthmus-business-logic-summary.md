...
> 该建议已细化为可执行方案：
> [契约对拍方案](2026-09-23_18-23-00-isthmus-contract-differential.md)。
> 核心结论是仓库里已有「真实 CLI × 合成上游」跑道
> （`isthmus-runtime/test/cli/`），只差把 `config.ts:3` 写死的 `CLI_PATH`
> 参数化，即可让同一请求穿过 2.1.258 与 2.1.280 两个版本做对拍——
> **零成本、不联网、不碰生产**。
