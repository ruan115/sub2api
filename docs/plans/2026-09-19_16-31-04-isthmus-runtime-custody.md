...
- **阶段 2a 完成**（`32de688`）：显式撤销接入命令路径。`RevokeEpoch`／`stop`／
  `destroy` 三条真实结束路径回收托管连接，`INSPECT` 不回收；回收严格按
  (槽位, epoch)，不触碰其他槽位与节点级控制连接。关键证据是对**真实 gRPC 监听器**
  证明传输确实拆除，而非只看注册表计数。
- **阶段 2b 完成**（`fc72a1b` 授权 + `88b56d8` 接线）：节点侧授权由
  `SessionAuthority` 提供，只用「控制会话是否打开」+「控制面推送的撤销水位线」，
  **host-agent 不持有任何数据库凭据**。review 发现初版「以 `Send` 成功判定新鲜度」
  不成立（`Send` 只进本地写缓冲；且无 gRPC keepalive，黑洞分区约 15 分钟才被 TCP
  发现），已加 keepalive 并改为记录会话开合、单调时钟。接线用 `nodeFacts` 后绑定
  打破「执行器→custody→authority→执行器」循环，未绑定窗口失败关闭。
- **用户指定修复顺序的三步**：
  1. `1b0db23` — **更正**「节点水位线重启 fail-open」的高估定性，并修掉核实中
     发现的真洞：签发门禁只校验 Redis 单库，会给**已撤销租约签出证书**；改接
     `Coordinator` 两库合取。
  2. `88b56d8` — 打破循环、接进 daemon，`Custody.Run` 有了非测试调用方。
  3. `26d16d9` — 双库回收链路验收测试已写、已编译，**但未验收**，见下。

## 6. 阶段 3 当前状态：远端已执行一次，**失败在配置不在代码**

2026-09-19 由用户在本会话用 `!` 执行远端同机运行，结果：

```
dial tcp 127.0.0.1:33379: connect: connection refused
```

`127.0.0.1:33379` 是**本地 SSH 隧道**地址；测试跑在**远端**，那里没有该监听。远端应
走私网地址（README 记录为 `172.19.0.2:3306` / `172.19.0.3:6379`，但同一文件也说明
私网 IP 重建后不保证不变）。**两个用例都没跑到断言就退出了**，因此：

- 双库回收链路**仍未实证**，阶段 3 **未验收**；
- 下一步是让远端运行时从容器动态取私网 IP 后再注入 DSN/URL（已给出命令），或把
  该二进制接进既有 `run-tests.sh` 的同机运行方式；
- 二进制已从 `26d16d9` 交叉编译（linux/amd64，sha256 前缀 `1d988191c05b707d`），
  **不可再用 `32de688` 的旧二进制冒充新代码验证**。

## 7. 安全事项（本会话新增）

2026-09-19 会话中出现了一把对应 **216.106.185.119** 的 SSH 私钥文本。未被使用、
未写入任何文件、未进入任何提交（推送前已扫描确认仓库内无私钥文件）。但它已进入
会话记录，按本项目「凭据不进聊天/文档/Git」的边界应视为**已泄露**：建议在 216 上
移除对应公钥并换发新密钥。216 本身在本轮全程未被连接、未被改动。
