#!/bin/sh
# Generate/check only worker.pb.go using already cached, pinned Go sources.
...
case "$mode" in
    check|generate) ;;
    *) echo 'usage: worker-proto-offline.sh [check|generate]' >&2; exit 2 ;;
esac
...
scratch_dir=$(mktemp -d /tmp/execution-worker-proto.XXXXXX)
cleanup() {
    case "$scratch_dir" in
        /tmp/execution-worker-proto.??????) rm -rf -- "$scratch_dir" ;;
    esac
}
...
"$scratch_dir/buf" generate --template "$template" --path api/proto/execution/v1/worker.proto --output "$scratch_dir/output"
generated=$scratch_dir/output/gen/go/execution/v1/worker.pb.go
target=$module_dir/gen/go/execution/v1/worker.pb.go
test -f "$generated"
