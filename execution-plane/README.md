...
construction/serving handoff is implemented behind the explicit enable flag. CCMAX now performs a generation-fenced completed-result projection and all six single-account material sources use the worker intake. CCMAX also emits a same-transaction secret-free proxy-reservation grant before the onboarding event, fences proxy/pool mutation and allocation while that authority is owned, and performs successor revoke/grant handoff without changing ordinary lifecycle semantics. The execution-plane projects/revokes that authority, reconciles same-image generation drift, and consumes all CCMAX runtime events through one ordered checkpoint. Onboarding events durably project the desired slot and a retryable start trigger; the coordinator atomically creates one trusted proxy lease plus workflow only after a fresh healthy assignment exists. CCMAX serializes outbox sequence allocation with a transaction-held singleton lock, and orchestrator startup rejects a missing lock row, malformed checkpoint or unsafe historical bootstrap. Authenticated/audited exact blocked-event retry, private route publication and duplicate-aware drain/archive batches are implemented. Token refresh, migration and production data-plane integration remain incomplete. These
