...
Docker-container Buildx 使用特权 BuildKit 容器，官方实现没有关闭该权限的
driver 参数。因此只允许固定官方 BuildKit 镜像、审查的 Dockerfile 和官方 Debian
...
5. 唯一 BuildKit builder，显式 Unix daemon endpoint、空0700 Docker配置目录；
   固定 builder digest、memory=2GiB/memory-swap=2GiB、cpu-quota=200000/
   cpu-period=100000、restart-policy=no。启动后核对真实 HostConfig 和 cgroupv2
   ...
   拉取仍可能联网，与离线包安装分开记证据。失败不运行APT联网修复。
