...
### 远端执行结果（2026-09-19，用户在会话中执行）

已按远端同机方式跑过一次，**失败在配置，不在代码**：

```
dial tcp 127.0.0.1:33379: connect: connection refused
```

`127.0.0.1:33379` 是**本地 SSH 隧道**地址，而测试运行在**远端**，那里无此监听；
远端应走容器私网地址。**两个用例都在建立连接时退出，未跑到任何断言**。

下一步：远端运行时从本轮自建的两个容器动态取私网 IP 再注入 DSN/URL（避免写死，
因 README 已注明私网 IP 重建后不保证不变），或把该二进制接进既有 `run-tests.sh`
的同机运行方式。
