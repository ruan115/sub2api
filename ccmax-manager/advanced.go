...
func (a *app) migrateAdvancedFeatures() error {
	...
	if err := addColumnIfMissing(a.db, "groups", "claude_cli_version", "TEXT NOT NULL DEFAULT '2.1.220'"); err != nil {
		return err
	}
	...
	if err := addColumnIfMissing(a.db, "groups", "force_cache_ttl_5m_enabled", "INTEGER NOT NULL DEFAULT 1"); err != nil {
		return err
	}
	// On by default: the anti-fingerprint dateline normalization stays active
	// for every group. Disable it only for a group that must forward the
	// client's exact token count (e.g. an external token-parity check),
