...
- 429 降权：账号**第一次**收到网关 429 就降低调度权重，不论当时配额用了多少。降权是本配额窗口内的事实，**成功请求不会解除**，只有两种方式消除：账号自己的 5h/7d 配额窗口自然刷新，或管理员手动恢复。连续 3 次 429 进一步从「降权」升级为「冷却」，冷却结束后账号恢复可调度但仍保持降权，直到窗口刷新。同一账号 1 分钟内的多次 429 只记 1 次（上游限流会同时打到全部在途请求）。
