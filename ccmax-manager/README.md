...
- 429 降权：**分组开关，默认开启**（取代原「429 重试等待」）。开启后账号**第一次**收到网关 429 就降低调度权重，不论当时配额用了多少。降权是本配额窗口内的事实，**成功请求不会解除**，只有两种方式消除：账号自己的 5h/7d 配额窗口自然刷新，或管理员手动恢复。累计到分组配置的**冷却阈值**（默认 3 次，可配 1-10）后从「降权」升级为「冷却」（暂不可调度）；填 1 表示首次 429 即冷却。冷却结束后账号恢复可调度但仍保持降权，直到窗口刷新。同一账号 1 分钟内的多次 429 只记 1 次（上游限流会同时打到全部在途请求）。关闭该开关**立即生效**：本组不再记录新的降权，也不再让已有降权影响调度顺序（降权状态按分组在调度时判定，因此同一账号可以在 A 组受降权约束、在 B 组完全不受影响）。无论开关如何，上游明确返回的冷却头（`retry-after` 与 5h/7d 配额窗口）始终生效，账号照常退避 —— 那是上游的指令而非本系统的启发式；但没有 `retry-after` 时的默认等待属于本系统的推测，随开关一同关闭。
