...
- 过载保护：分组可配置 529 的账号+模型短熔断，默认 10 秒；网关 429 会降低账号调度权重，连续 3 次后冷却到下一个 5h 窗口刷新，支持管理员手动恢复。同一账号 1 分钟内的多次 429 只记 1 次（上游限流会同时打到全部在途请求），记分超过 5 小时未再触发则自动清零。上游明确回报 5h/7d 配额窗口已满时按该窗口冷却，且成功请求不会解除这类封禁。
